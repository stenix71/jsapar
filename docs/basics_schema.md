...
* If the type is string then the pattern should contain a regular expression to which the value is validated against. 
  See [java.util.regex.Pattern](https://docs.oracle.com/javase/8/docs/api/java/util/regex/Pattern.html) This only works while parsing.
...
* If the type is one of the new java.time types such as local_date, local_date_time, zoned_date_time or instant, then the 
 pattern should be described according to [java.time.format.DateTimeFormatter](https://docs.oracle.com/en/java/javase/17/docs/api/java.base/java/time/format/DateTimeFormatter.html). If a time-zone is required such as 
 when parsing an instant, you can add it within the pattern separated with a `@` character. Example: `pattern="yyyy-MM-dd HH:mm@Europe/Stockholm"`
* If the type is one of duration or period, then the
  pattern should be described according to [java.time.format.DateTimeFormatter](https://docs.oracle.com/en/java/javase/17/docs/api/java.base/java/time/format/DateTimeFormatter.html). 
  For duration, only the time related symbols are allowed, not days months etc. For period, only the date related 
  symbols are allowed. If no 
  pattern is specified, the default parse/toString format of the java Duration and Period classes are used.
