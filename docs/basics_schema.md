...
* If the type is one of duration or period, then the
  ...
  For duration, only the time related symbols are allowed. For period, only the date related symbols are allowed. If no pattern is specified, the default parse format of the java Duration and Period classes are used.
